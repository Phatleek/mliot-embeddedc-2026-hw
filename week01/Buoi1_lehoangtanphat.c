...
void parse_config(const uint8_t *config_packet, int16_t *high_threshold) {
    ...
    if (high_threshold == NULL)
        return;
int16_t byte_thap = config_packet[0]; // byte dau tien
int16_t byte_cao  = config_packet[1]; // byte sau

 int16_t byte_cao_da_dich = byte_cao << 8;
 int16_t ans = byte_cao_da_dich + byte_thap;
 int16_t ans = byte_cao_da_dich + byte_thap;
 *high_threshold = ket_qua_gop;
...
int16_t read_temperature_reg(void *hw_sensor_reg) {
    ...
    if (hw_sensor_reg == NULL) {
     ...
     volatile int16_t *cam_bien;
    cam_bien = (volatile int16_t *)hw_sensor_reg;


    int16_t gia_tri_nhiet_do_hien_tai;
    gia_tri_nhiet_do_hien_tai = *cam_bien;
    return gia_tri_nhiet_do_hien_tai;
...
void control_output(uint8_t *control_reg, uint8_t fan_enable, uint8_t alarm_enable) {
   ...
   if (control_reg == NULL) {
    ...
    if (fan_enable == 1) {
        *control_reg = *control_reg | (1 << PIN_0);
    }
    if (fan_enable == 0) {
        *control_reg = *control_reg & ~(1 << PIN_0);
    }
    ...
    if (alarm_enable == 1) {
        *control_reg = *control_reg | (1 << PIN_1);
    }
    if (alarm_enable == 0) {
        *control_reg = *control_reg & ~(1 << PIN_1);
    }
